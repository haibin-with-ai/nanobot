...
## 第二轮审查后的口径拍板（2026-07-29）

8. **stall 放弃走错误出口，不抛异常。** Phase 3 原本抛 `ModelStallError`，但生产链路没有任何 catch，会把整个回合炸掉、上下文不落盘、cron 任务静默失败。改成落到通用错误分支：`stop_reason="error"`，文案「模型连续多次无响应，本轮放弃」，历史照常保存。

9. **stall 计数在任何有响应的回合归零，包括只调工具的回合。** 原实现只在拿到最终答案时清账，长任务里跨几十个工具回合累计四次超时就会提前放弃。

10. **拒答不推熔断器。** 拒答触发换模型，但不计入主模型故障计数，否则三次内容拒答会把主模型摘掉十分钟。

11. **已经吐字的流不重试、不换模型。** 流式路径中途 401 或拒答时，用户已经看到部分输出，重试会产生重复或自相矛盾的两段话。只有在还没有任何 delta 时才自愈。

12. **stall 后重建 client 只换引用，不 close 旧连接。** provider 实例在会话间共享，旧 client 上可能挂着别人的在途流。

13. **外层墙钟只放大一倍，不按整条链长度放大。** 这道 `wait_for` 是保命墙不是 SLA：给备用模型留一个模型的余量即可，六个候选不该让用户干等六倍。

14. **per-run cron 会话回收改在造文件的地方节流执行。** 除 gateway 启动扫一次外，每次未绑定 cron 触发时调用 `maybe_prune_cron_run_sessions()`，一天最多真扫一次，常驻数月的实例不再无限累积。

15. **只有 session_key 缺 origin 的任务降级时必须留日志。** 静默降级会让「回到原会话回复」的任务一直答到别处去。
