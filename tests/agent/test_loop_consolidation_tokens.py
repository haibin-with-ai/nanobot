...
async def test_prompt_below_threshold_does_not_consolidate(tmp_path) -> None:
    ...
    loop.memory_consolidator.archive = AsyncMock(return_value=True)  # type: ignore[method-assign]

    await loop.process_direct("hello", session_key="cli:test")

    loop.memory_consolidator.archive.assert_not_awaited()
...
async def test_prompt_above_threshold_triggers_consolidation(tmp_path, monkeypatch) -> None:
    ...
    assert loop.memory_consolidator.archive.await_count >= 1
...
async def test_prompt_above_threshold_archives_until_next_user_boundary(tmp_path, monkeypatch) -> None:
    ...
    archived_chunk = loop.memory_consolidator.archive.await_args.args[0]
...
async def test_consolidation_loops_until_target_met(tmp_path, monkeypatch) -> None:
    ...
    assert loop.memory_consolidator.archive.await_count == 2
...
async def test_preflight_consolidation_before_llm_call(tmp_path, monkeypatch) -> None:
    ...
    async def track_consolidate(messages):
        ...
        return True
    loop.memory_consolidator.archive = track_consolidate  # type: ignore[method-assign]
...
async def test_trigger_ratio_0_5_fires_earlier(tmp_path) -> None:
    """With ratio=0.5, a 300-token estimate against a 500-token window should fire."""
    loop = _make_loop(tmp_path, estimated_tokens=300, context_window_tokens=500)
    ...
    await loop.process_direct("hello", session_key="cli:test")
    loop.memory_consolidator.archive.assert_awaited()


@pytest.mark.asyncio
async def test_trigger_ratio_0_5_does_not_fire_when_below_budget(tmp_path) -> None:
    """With ratio=0.5, a 200-token estimate against a 500-token window should NOT fire."""
    loop = _make_loop(tmp_path, estimated_tokens=200, context_window_tokens=500)
    loop.memory_consolidator.trigger_ratio = 0.5
    loop.memory_consolidator._SAFETY_BUFFER = 0
    loop.memory_consolidator.archive = AsyncMock(return_value=True)  # type: ignore[method-assign]
