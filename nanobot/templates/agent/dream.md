...
- Short sessions often contain process noise, not durable memory. Prefer no write over creating over-fine memories from one-off commands, transient debugging, or ordinary execution logs.
...
## Completion contract
- Do NOT touch `memory/.dream_cursor` — the runtime advances it automatically after a successful run.
- Finish all required memory edits, then end the run normally. No extra summary is required; if you do reply, keep it short.
