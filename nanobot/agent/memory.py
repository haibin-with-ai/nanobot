...
# Single source of truth for the staleness threshold used in line-age annotations
# and in the Dream prompt template (passed as `stale_threshold_days`).
...
class MemoryStore:
    ...
    def build_dream_prompt(
        ...
        *,
        ...
        """Build the single-phase Dream prompt from unprocessed history."""
        ...
        history_text = "\n".join(
            f"[{e['timestamp']}] "
            f"{truncate_text(e['content'], _HISTORY_ENTRY_PREVIEW_MAX_CHARS)}"
            for e in batch
        )
        raw_memory = self.read_memory() or "(empty)"
        memory_text = (
            self._annotate_memory_line_ages(raw_memory)
            if annotate_line_ages
            else raw_memory
        )
        memory_text = truncate_text(memory_text, _MEMORY_FILE_MAX_CHARS)
        soul_text = truncate_text(self.read_soul() or "(empty)", _SOUL_FILE_MAX_CHARS)
        user_text = truncate_text(self.read_user() or "(empty)", _USER_FILE_MAX_CHARS)
        ...
        prompt = (
            ...
            f"## Current Date\n{current_date}\n\n"
            f"## Target Cursor\n{batch[-1]['cursor']}\n\n"
            f"## Current MEMORY.md ({len(memory_text)} chars)\n{memory_text}\n\n"
            f"## Current SOUL.md ({len(soul_text)} chars)\n{soul_text}\n\n"
            f"## Current USER.md ({len(user_text)} chars)\n{user_text}\n\n"
    ...
    def _annotate_memory_line_ages(self, content: str) -> str:
        ...
        try:
            ages = self.git.line_ages(file_path)
        except Exception:
    ...
    def prune_dream_sessions(sessions_dir: Path, *, keep: int = 10) -> None:
        ...
        if len(dream_files) <= keep:
            return
        for path in dream_files[: len(dream_files) - keep]:
